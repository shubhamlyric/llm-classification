...
def process_and_predict_multiple(
    new_data: pl.DataFrame,
    faiss_storage: FaissStorage,
    prompt_template,
    parameters: Parameters,
    ...
    for model_name in model_names:
        predictions[f"prediction_{model_name}"] = []
    ...
    for row in new_data.iter_rows(named=True):
        ...
        for agent, model_name in zip(agents, model_names):
            agent_prediction = agent.run(prompt).strip()
            final_prediction = 1 if agent_prediction.lower() in ['1', 'true', 'yes', 'survived'] else 0
            predictions[f"prediction_{model_name}"].append(final_prediction)

    # Add prediction columns to the original dataframe
    for model_name in model_names:
        original_data = original_data.with_columns(pl.Series(f"prediction_{model_name}", predictions[f"prediction_{model_name}"]))
